...
gulp.task('sass', function() {
  return gulp.src(['./scss/*.scss', './scss/**/*.scss', '!**/_*/**', '!./scss/components/*'])
    .pipe(sassGlob())
    .pipe(sass())
    .pipe(gulp.dest('./css'));
});
...
async function screenshots(dirs = []) {
	...
	*/ 
	...
	for (i in sections) {
		...
		let html = `<html><head><base href="../../"><link href="https://fonts.googleapis.com/css2?family=Nunito:wght@300;500;700&amp;family=Inter:wght@400;700&amp;" rel="stylesheet"><link href="/css/bootstrap.min.css" rel="stylesheet"><link href="/css/style.css" rel="stylesheet"></head><body>${content}</body></html>`;
...
function sections(type = "section", dirs = []) {
	...
	for (i in dir) {
	  ...
	  let filePath = `${sectionsDir}/${fileName}`;
		...
		let id;
		let html;
		let sectionsHtml = '<html><head><base src="../"><link href="../css/bootstrap.min.css" rel="stylesheet"><link href="../css/style.css" rel="stylesheet"></head><body>';
		let image = '';
